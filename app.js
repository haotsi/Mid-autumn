...
let timer;const wishes=['但愿人长久，千里共婵娟。','愿此刻月光，照亮每一份思念。','桂香入梦，月满人间。中秋快乐。'];let wi=0;function wish(){const el=document.querySelector('#blessing');el.textContent=wishes[wi++%wishes.length];el.classList.add('show');clearTimeout(timer);timer=setTimeout(()=>el.classList.remove('show'),5000);}document.querySelector('#wish').onclick=wish;document.querySelector('#reset').onclick=reset;document.querySelector('.brand').onclick=e=>{e.preventDefault();reset();};
